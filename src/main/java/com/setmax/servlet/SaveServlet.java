...
public class SaveServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		StudentDetails student = new StudentDetails(fullname, mothername, fathername, standard, fees, phoneno);
		PrintWriter writer = response.getWriter();
		StudentDetailService studentDetailService = new StudentDetailService();
		int status = studentDetailService.save(student);

		if (status > 0) {
			response.sendRedirect("viewServlet");
		} else {
			writer.println("Sorry! Unable to save record in database!");
